...
public class LocalStack extends Stack {
	...
	public LocalStack(final App scope, final String id, final StackProps props) {
		...
		FargateService billingService = createFargateService("BillingService", "billing-service", List.of(4001, 9001),
				null, null);

		FargateService analyticsService = createFargateService("AnalyticsService", "analytics-service", List.of(4002),
				null, null);

		analyticsService.getNode().addDependency(mskCluster);

		FargateService patientService = createFargateService("PatientService", "patient-service", List.of(4000),
				patientServiceDb, Map.of("BILLING_SERVICE_ADDRESS", "billing-service.patient-management.local",
	...
	private DatabaseInstance createDatabase(String id, String dbName) {
		return DatabaseInstance.Builder.create(this, id)
				...
				.allocatedStorage(20).credentials(Credentials.fromGeneratedSecret("admin_user")).databaseName(dbName)
				.removalPolicy(RemovalPolicy.DESTROY).build();
	}

	private CfnHealthCheck createDbHealthCheck(DatabaseInstance db, String id) {
		return CfnHealthCheck.Builder.create(this, id)
				.healthCheckConfig(CfnHealthCheck.HealthCheckConfigProperty.builder().type("TCP")
						.port(Token.asNumber(db.getDbInstanceEndpointPort()))
						.ipAddress(db.getDbInstanceEndpointAddress()).requestInterval(30).failureThreshold(3).build())
				.build();
	}

	private CfnCluster createMskCluster() {
		return CfnCluster.Builder.create(this, "MskCluster").clusterName("kafa-cluster").kafkaVersion("3.8.x")
	...
	// Service Discovery. defaultCloudMapNamespace not supported very well on
	// localstack
	...
	private FargateService createFargateService(String id, String imageName, List<Integer> ports, DatabaseInstance db,
		...
		FargateTaskDefinition taskDefinition = FargateTaskDefinition.Builder.create(this, id + "Task").cpu(256)
				.memoryLimitMiB(512).build();

		ContainerDefinitionOptions.Builder containerOptions = ContainerDefinitionOptions.builder()
				.image(ContainerImage.fromRegistry(imageName))
				.portMappings(ports.stream()
						.map(port -> PortMapping.builder().containerPort(port).hostPort(port).protocol(Protocol.TCP)
								.build())
						.toList())
		...
		FargateService service = FargateService.Builder.create(this, id).cluster(ecsCluster)
				.taskDefinition(taskDefinition).assignPublicIp(false).serviceName(imageName)
				.cloudMapOptions(CloudMapOptions.builder() // ← NEW
						.name(imageName) // ← NEW
						.cloudMapNamespace(ecsCluster.getDefaultCloudMapNamespace()) // ← NEW
						.dnsRecordType(DnsRecordType.A) // ← NEW
						.dnsTtl(Duration.seconds(60)) // ← NEW
						.build()) // ← NEW
				.build();
	...
	private void createApiGatewayService() {
		...
		ContainerDefinitionOptions containerOptions = ContainerDefinitionOptions.builder()
				...
				.environment(Map.of("SPRING_PROFILES_ACTIVE", "prod", "AUTH_SERVICE_URL", // prod - looks for
																							// application.yml with prod
																							// in file name
						"http://auth-service.patient-management.local:4005")) // since localstack does not support
				// service discovery very
				// well, just use the docker inyernal discovery
		...
		ApplicationLoadBalancedFargateService apiGateway = ApplicationLoadBalancedFargateService.Builder
				.create(this, "APIGatewayService").cluster(ecsCluster).serviceName("api-gateway")
				.taskDefinition(taskDefinition).desiredCount(1).healthCheckGracePeriod(Duration.seconds(60)).build();
	}
